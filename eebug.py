...
def aws_network_graph(eni_details, subnet_details, route_table_details, tgw_details, tgw_attachments):
    ...
    elements.append({'data': {'id': 'eni', 'label': f'ENI {eni_id}', 'type': 'ENI', 'icon': 'aws:network-adapter'}})
    elements.append({'data': {'id': 'subnet', 'label': f'Subnet {subnet_id}', 'icon': 'aws:network' }})
    elements.append({'data': {'id': 'route_table', 'label': f'Route Table {route_table_id}', 'icon': 'aws:route-table' }})
    elements.append({'data': {'id': 'tgw', 'label': f'Transit Gateway {tgw_id}', 'icon': 'aws:transit-gateway' }})
    elements.append({'data': {'id': 'tgw_rtb', 'label': f'Tgw Route Table ID {tgw_route_table_id}', 'icon': 'aws:route-table' }})
    elements.append({'data': {'id': 'tgw_attachment', 'label': f'Transit Gateway Attachment {tgw_attachment_id}', 'icon': 'aws:attachment' }})
            ...
            dbc.Col(
                Cytoscape(
                    ...
                    stylesheet=[
                        ...
                        {'selector': '[type = "ENI"]', 'style': {'background-color': '#6FB1FC', 'border-color': '#3573A5'}},
                        {'selector': '[type = "Subnet"]', 'style': {'background-color': '#98FB98', 'border-color': '#4CAF50'}},
                        {'selector': '[type = "Route Table"]', 'style': {'background-color': '#FFD700', 'border-color': '#FFC107'}},
                        {'selector': '[type = "Transit Gateway"]', 'style': {'background-color': '#FF6347', 'border-color': '#E57373'}},
                        {'selector': '[type = "Transit Gateway Attachment"]', 'style': {'background-color': '#8A2BE2', 'border-color': '#7B1FA2'}},
